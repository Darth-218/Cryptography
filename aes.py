...
# Converts a 16-byte string into a 4x4 matrix
...
# Subistitutes bytes using the s-box
def sub_bytes(s): 
    for i in range(4):
        for j in range(4):
            s[i][j] = SBOX[s[i][j]]
...
# Multiplies a byte by 2 in GF(2^8) (is used because of overflow in AES)
def xtime(a): return ((a << 1) ^ 0x1B) & 0xFF if a & 0x80 else a << 1

# Mixes columns of the state matrix according to AES specification
def mix_columns(s):
    for i in range(4):
...
# Generates the round keys for each round
def key_expansion(key):
    ...
    for i in range(4, 44):
        ...
        if i % 4 == 0:
            ...
            word[0] ^= RCON[i//4]
        new_word = xor_bytes(keys[i-4], word)
        keys.append(new_word)
    return [keys[4*i:4*i+4] for i in range(11)]
